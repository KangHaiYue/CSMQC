...
class QuantumSim:
    def __init__(self, labels):
        ...
        # path = './noise_dict_27q.pkl'
        path = "./noise_dict.pkl"
        with open(path, "rb") as f:
            error_dict = pickle.load(f) #error_dict = {('Gcnot',0,1):{'SX:2':0.001231, 'SXZ:2,3':0.000213}} #keys are the gates, values are the induced errors probability
        str_labels = [str(x) for x in labels]
        new_error_dict = {}
        # filter out unused qubits
        for k, v in error_dict.items():
            #ignore this induced error since k (the gate) is not included in the circuit, so will not be induced
            if not (k[1] in labels and k[2] in labels):
                continue
            new_v = {}
            for k_, v_ in v.items():
                v_labels = k_.split(":")[1].split(",")
                #add to the new error dictionary only if induced error gates acts on the qubits allowed in the circuit
                if len(set(str_labels) & set(v_labels)) == len(v_labels):
                    new_v[k_] = v_
            if len(new_v) > 0:
                new_error_dict[k] = new_v

            #for k2, v2 in error_dict.items():
            #    if (k2[1] == k[1]) or (k2[1] == k[2]) or (k2[2] == k[1]) or (k2[2] == k[2]) or not (k2[1] in labels and k2[2] in labels):
            #        continue
            #    
            #    new_v_2_sources = {}
            #    for k1_, v1_ in v.items():
            #        for k2_, v2_ in v2.items():
            #            v2_labels = k2_.split(":")[1].split(",")
            #            if k1_ == k2_ and (len(set(str_labels) & set(v2_labels)) == len(v2_labels)):
            #                new_v_2_sources[k2_] = v1_ + v2_
            #    if len(new_v_2_sources) > 0:
            #        new_error_dict[('Gcnot2', k[1], k[2], k2[1], k2[2])] = new_v_2_sources
                
        #set the single-qubit HSA error of qubit 11 (DQ) to be a fixed ratio to 14 (SQ)
        #new_error_dict[('Gcnot', 13, 12)]['SX:11'] = new_error_dict[('Gcnot', 13, 12)]['SX:14'] = 0
        #new_error_dict[('Gcnot', 13, 12)]['SY:11'] = new_error_dict[('Gcnot', 13, 12)]['SY:14'] = 0
        #new_error_dict[('Gcnot', 13, 12)]['SZ:11'] = new_error_dict[('Gcnot', 13, 12)]['SZ:14'] = 0

        #new_error_dict[('Gcnot', 13, 12)]['AX:11'] = new_error_dict[('Gcnot', 13, 12)]['AX:14'] = 0
        #new_error_dict[('Gcnot', 13, 12)]['AY:11'] = new_error_dict[('Gcnot', 13, 12)]['AY:14'] = 0
        #new_error_dict[('Gcnot', 13, 12)]['AZ:11'] = new_error_dict[('Gcnot', 13, 12)]['AZ:14'] = 0

        #new_error_dict[('Gcnot', 13, 12)]['HX:11'] = new_error_dict[('Gcnot', 13, 12)]['HX:14']/5
        #new_error_dict[('Gcnot', 13, 12)]['HY:11'] = new_error_dict[('Gcnot', 13, 12)]['HY:14']/5 
        #new_error_dict[('Gcnot', 13, 12)]['HZ:11'] = new_error_dict[('Gcnot', 13, 12)]['HZ:14'] = 0
        
        #if neglect the two-qubit HSA error 
        #for i in ['X', 'Y', 'Z']:
        #    for j in ['X', 'Y', 'Z']:
        #        new_error_dict[('Gcnot', 13, 12)]['S'+i+j+':11,14'] = 0
        #        new_error_dict[('Gcnot', 13, 12)]['A'+i+j+':11,14'] = 0
        #        new_error_dict[('Gcnot', 13, 12)]['H'+i+j+':11,14'] = 0
            
        
        #amplify the HSA error rates
        #for error_type in new_error_dict[('Gcnot', 0, 1)].keys():
        #    if error_type[0] == 'H' and len(error_type.split(':')[1].split(',')) == 1:
        #        new_error_dict[('Gcnot', 0, 1)][error_type] *= 1.663
        #    new_error_dict[('Gcnot', 0, 1)][error_type] *= 2.173
        
            
            #elif error_type[0] == 'H' and len(error_type.split(':')[1].split(',')) == 2:
            #    new_error_dict[('Gcnot', 0, 1)][error_type] = 0
            #else:
            #    new_error_dict[('Gcnot', 0, 1)][error_type] = 0
        
        #for error_type in new_error_dict[('Gcnot', 1, 0)].keys():
        #    if error_type[0] == 'H' and len(error_type.split(':')[1].split(',')) == 1:
        #        new_error_dict[('Gcnot', 1, 0)][error_type] *= 1.663
        #    new_error_dict[('Gcnot', 1, 0)][error_type] *= 2.173
            
            #elif error_type[0] == 'H' and len(error_type.split(':')[1].split(',')) == 2:
            #    new_error_dict[('Gcnot', 1, 0)][error_type] = 0
            
        
        ######
        CX_err_1 = error_dict[('Gcnot', 0, 1)]
        CX_err_2 = error_dict[('Gcnot', 1, 0)]
        CX_err_1_new = {}
        CX_err_2_new = {}
        for k_, v_ in CX_err_1.items():
            v_labels = k_.split(":")[1].split(",")
            #add to the new error dictionary only if induced error gates acts on the qubits allowed in the circuit
            if len(set(str_labels) & set(v_labels)) == len(v_labels):
                if k_[0] == 'H' and len(k_.split(':')[1].split(',')) == 1:
                    CX_err_1_new[k_] = v_*2.54
                else:
                    CX_err_1_new[k_] = v_*2.54
        
        for k_, v_ in CX_err_2.items():
            v_labels = k_.split(":")[1].split(",")
            #add to the new error dictionary only if induced error gates acts on the qubits allowed in the circuit
            if len(set(str_labels) & set(v_labels)) == len(v_labels):
                if k_[0] == 'H' and len(k_.split(':')[1].split(',')) == 1:
                    CX_err_2_new[k_] = v_*2.54
                else:
                    CX_err_2_new[k_] = v_*2.54
        
        
        #new_error_dict[('Gcnot', 4, 8)] = CX_err_1_new
        #new_error_dict[('Gcnot', 8, 4)] = CX_err_2_new
        
        new_error_dict[('Gcnot', 0, 1)] = CX_err_1_new
        new_error_dict[('Gcnot', 1, 0)] = CX_err_2_new
        
        self.new_error_dict = new_error_dict
        self.mdl = pygsti.models.create_cloud_crosstalk_model(pspec, lindblad_error_coeffs=new_error_dict)#crosstalk error model should be included in new_error_dict
